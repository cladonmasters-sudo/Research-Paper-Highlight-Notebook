...
async function exportData() {
    ...
    for (const category in notes) {
        ...
        notes[category].forEach(rawNote => {
            const note = normalizeNote(rawNote);
            const plainNote = note.text.replace(/<[^>]*>?/gm, " ");

            children.push(new Paragraph({
                children: [new TextRun({
                    text: `${note.article || "No article"} | Page: ${note.page || "N/A"} | ${plainNote}`,
                    size: 22
                })]
            }));
        });
    ...
    children.push(new Paragraph({
        children: [new TextRun({ text: "Saved Citations", bold: true, size: 28 })]
    }));

    citations.forEach(citation => {
        children.push(new Paragraph(String(citation)));
    });
